...
describe("Plugin: datadog (log)", function()
  ...
  it("logs metrics without api-name tag when set to false", function()
    local thread = threads.new({
      function()
        local socket = require "socket"
        local server = assert(socket.udp())
    ...
    assert.True(ok)
    assert.equal(3, #gauges)
    assert.contains("kong.dd6.request.status.200:1|c", gauges)
    assert.contains("kong.dd6.request.status.total:1|c", gauges)
    assert.contains("kong.dd6.latency:%d+|g|#T2:V2:V3,T4", gauges, true)
  end)
